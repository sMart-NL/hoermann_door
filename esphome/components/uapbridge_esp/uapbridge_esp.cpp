...
void UAPBridge_esp::receive() {
  ...
  if (newData) {
    ...
    // 🔥 MOD: ESP32 SLAVE SCAN (0x81 ipv 0x28)
    // Slave scan - 81 82 01 80 06
    if (this->rx_data[0] == ESP32_SLAVE_ADDR) {  // ← DIT IS DE MOD!
      ...
      if (this->rx_data[2] == CMD_SLAVE_SCAN && this->rx_data[3] == UAP1_ADDR_MASTER && length == 2 && calc_crc8(this->rx_data, length + 3) == 0x00) {
        ESP_LOGVV(TAG, "ESP32 SlaveScan: %s", print_data(this->rx_data, 0, 5));
        ESP_LOGV(TAG, "->      ESP32 SlaveScan"); 
        counter = (this->rx_data[1] & 0xF0) + 0x10;
        this->tx_data[0] = UAP1_ADDR_MASTER;
        this->tx_data[1] = 0x02 | counter;
        this->tx_data[2] = UAP1_TYPE;
        this->tx_data[3] = ESP32_SLAVE_ADDR;     // ← OOK HIER!
    ...
    // Broadcast status - 00 92 12 02 35
    ...
    // 🔥 MOD: ESP32 Status Request (0x81 ipv 0x28)
    // Slave status request - 81 A1 20 2E
    if (this->rx_data[1] == ESP32_SLAVE_ADDR) {  // ← DIT IS DE MOD!
      length = this->rx_data[2] & 0x0F;
      if (this->rx_data[3] == CMD_SLAVE_STATUS_REQUEST && length == 1 && calc_crc8(&this->rx_data[1], length + 3) == 0x00) {
        ESP_LOGVV(TAG, "ESP32 Slave status request: %s", print_data(this->rx_data, 1, 5));
        ESP_LOGV(TAG, "->      ESP32 Slave status request");
...
void UAPBridge_esp::transmit() {
  ...
  this->parent_->set_parity(esphome::uart::UARTConfig::NONE);
  this->parent_->set_stop_bits(1);
  this->parent_->load_settings(false);
...
// Action methods blijven hetzelfde...
...
void UAPBridge_esp::handle_state_change(hoermann_state_t new_state) {
  ...
  switch (new_state) {
    case hoermann_state_open: this->state_string = "Open"; break;
    case hoermann_state_closed: this->state_string = "Closed"; break;
    case hoermann_state_opening: this->state_string = "Opening"; break;
    case hoermann_state_closing: this->state_string = "Closing"; break;
    case hoermann_state_venting: this->state_string = "Venting"; break;
    case hoermann_state_stopped: this->state_string = "Stopped"; break;
    default: this->state_string = "Error"; break;
  }
  this->data_has_changed = true;
}
